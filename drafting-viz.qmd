...
sunflower_oil %>% 
  ...
  theme(plot.background = element_rect(fill = "#B1D7FF"),
    panel.grid = element_blank(),
        ...
        axis.text.y.right = element_text(family = "geist_mono", size = rel(0.65), margin = margin(l = -15)),
...
ggsave("sunflower_oil_area.pdf", width  = 10)
...
ggsave("sunflower_seed_area.pdf", width  = 10)
...
```{r}
# create color scheme
exports <- c("Sunflower Oil" = alpha(dark_yellow, 0.7),
             "Maize" = alpha("#007756",0.7),
             "Wheat" = alpha("#846E54",0.7),
             "Rapeseed" = alpha("#6d7700", 0.7),
             "Barley" = alpha("#442824", 0.7))
...
ggplot(uki_agri_2021, aes(fill = reorder(export, -value_billions), values = value_billions)) + 
  waffle::geom_waffle(n_rows = 10, radius = unit(0.5, "npc"), size = 0,
                      flip = TRUE) +
  labs(fill = " ",
       title = "Main Export Products in 2021 (USD)",
